...
file_path = "/home/gwswdp02/data/zyh/trajs/SBM.json"
with open(file_path, 'r') as file:
...
output_feature_file = os.path.join(folder_path, 'feature', 'features_zyh_SBM.csv')
finaldata.to_csv(output_feature_file,index=False)

Mode = pd.DataFrame(modes, columns=["Mode"]) 
output_mode_file = os.path.join(folder_path, 'feature', 'mode_name_zyh_SBM.csv')
